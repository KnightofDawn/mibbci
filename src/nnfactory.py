...
def create_nn(
    ...
    if 'gtec' in nn_type:
        if 'CovMat' in nn_type:
            ...
            elif 'large' in nn_type:
                nnet, layer_obj = create_nn_gtec_CovMat_large(nn_input_shape, nn_output_shape, num_max_training_epochs)
        elif 'Img' in nn_type:
            if 'small' in nn_type:
...
def create_nn_gtec_CovMat_large(nn_input_shape, nn_output_shape, num_max_training_epochs):
    layer_obj = lasagne.layers.InputLayer(shape=nn_input_shape, name='Input')
    # To Conv1DLayer: 3D tensor, with shape (batch_size, num_input_channels, input_length)
