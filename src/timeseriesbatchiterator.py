    ...
    def __init__(self, data, labels=None, *args, **kwargs):
        ...
        if len(self._X.shape) == 3:    # Conv2D case
            ...
            self._X_buf = np.zeros(
                    [self.batch_size, self._X.shape[0], params.NUM_CHANNELS, params.WINDOW_SIZE_DECIMATED_SAMPLES],
            ...
            self._X_buf = np.zeros(
                    [self.batch_size, params.NUM_CHANNELS, params.WINDOW_SIZE_DECIMATED_SAMPLES],
                    np.float32)   # Pre-allocating buffers
    ...
    def transform(self, X_indices, y_indices):
        ...
        if len(self._X.shape) == 3:     # Conv2D case
            for i_index, i_time in enumerate(X_indices):
                ...
                if i_time < 0:
                    i_time = np.random.randint((params.WINDOW_SIZE_DECIMATED_SAMPLES-1), self._X.shape[1])

                X_window = self._X_padded[:, i_time:i_time+params.WINDOW_SIZE_DECIMATED_SAMPLES]
            ...
            for i_index, i_time in enumerate(X_indices):
                if i_time < 0:
                    i_time = np.random.randint((params.WINDOW_SIZE_DECIMATED_SAMPLES-1), self._X.shape[0])
                X_window = self._X_padded[i_time:i_time+params.WINDOW_SIZE_DECIMATED_SAMPLES]
                #print(TAG, "X_window part:\n", X_window[0, 2:6, 20:25])
                #print("X_window size:", X_window.shape)
